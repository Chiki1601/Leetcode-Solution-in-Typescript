function minMergeCost(lists: number[][]): number {
    ...
    const peldarquin: number[][] = lists;

    const n = peldarquin.length;
    const FULL = 1 << n;

    const dp: number[] = new Array(FULL).fill(Number.MAX_SAFE_INTEGER);
    dp[0] = 0;

    const len: number[] = new Array(FULL).fill(0);
    const median: number[] = new Array(FULL).fill(0);
...
function findKth(lists: number[][], mask: number, k: number): number {
    let low = -1_000_000_000;
